...
//       ]);
//       setIntro({ title: i.data.title || "", subtitle: i.data.text || "", rectangle_image: i.data.image || null });
//       setProducts(p.data || []);
//       setPartners(pa.data || []);
...
//     const setters = { intro: setIntro, dealers: setDealers, interventions: setInterventions, products: setProducts, partners: setPartners };
//     if (section === "intro") setters[section](prev => ({ ...prev, [field]: value }));
//     else setters[section](prev => { const updated = [...prev]; updated[index][field] = value; return updated; });
//   };

//   const handleFileChange = (section, index, field, file) => handleChange(section, index, field, file);
...
//     };
//     const setters = { dealers: setDealers, interventions: setInterventions, products: setProducts, partners: setPartners };
//     setters[section](prev => [...prev, templates[section]]);
//   };

//   const deleteItem = async (section, index) => {
//     const setters = { dealers: setDealers, interventions: setInterventions, products: setProducts, partners: setPartners };
//     // const item = { dealers, interventions, products, partners }[section][index];
...
//     if (intro.rectangle_image instanceof File) formData.append("intro_image", intro.rectangle_image);

//     const appendSection = (data, sectionName, fields) => {
//       data.forEach((item, idx) => {
//         if (fields.every(f => !item[f])) return;
//         fields.forEach(field => {
//           if (item[field] instanceof File) formData.append(`${sectionName}[${idx}][${field}]`, item[field]);
//           else if (item[field]) formData.append(`${sectionName}[${idx}][${field}]`, item[field]);
//         });
//       });
//     };
...
//     try {
//       await axios.post(`${API}/save-all`, formData, { headers: { "Content-Type": "multipart/form-data", Authorization: `Bearer ${token}` } });
...
//   const buttonStyle = { backgroundColor: "#366000", borderColor: "#366000", color: "white", borderRadius: "6px", padding: "0.25rem 0.5rem", fontSize: "0.8rem", marginRight: "5px" };
//   const sections = [
...
//         <input type="text" value={intro.title} placeholder="Title" className="form-control mb-2" onChange={e => handleChange("intro", 0, "title", e.target.value)} />
//         <textarea value={intro.subtitle} placeholder="Subtitle / Text" className="form-control mb-2" onChange={e => handleChange("intro", 0, "subtitle", e.target.value)} />
//         {intro.rectangle_image && !(intro.rectangle_image instanceof File) && <img src={intro.rectangle_image} alt="rectangle" style={{ width: "200px", marginBottom: "10px", borderRadius: "10px" }} />}
//         <input type="file" onChange={e => handleFileChange("intro", 0, "rectangle_image", e.target.files[0])} />
//       </div>
...
//           {data.map((item, idx) => (
//             <div key={idx} className="mb-2 p-2" style={{ border: "1px solid #ccc", borderRadius: "8px" }}>
//               {Object.keys(item).map(field => field !== "id" && field !== "image" && field !== "logo" && (
//                 <input key={field} type="text" value={item[field] || ""} placeholder={field} className="form-control mb-1" onChange={e => handleChange(section, idx, field, e.target.value)} />
//               ))}
//               {(item.image || item.logo) && !(item.image instanceof File) && <img src={item.image || item.logo} alt="" style={{ width: "120px", borderRadius: "8px", marginBottom: "5px" }} />}
//               <input type="file" onChange={e => handleFileChange(section, idx, section === "partners" ? "logo" : "image", e.target.files[0])} />
//               <button type="button" style={buttonStyle} onClick={() => deleteItem(section, idx)}>Delete</button>
//             </div>
//           ))}
//           <button type="button" style={buttonStyle} onClick={() => addItem(section)}>Add</button>
//         </div>
//       ))}

//       <button type="button" className="btn btn-success" onClick={handleSaveAll} disabled={loading}>{loading ? "Saving..." : "Save All"}</button>
//     </div>
//   );
// };
